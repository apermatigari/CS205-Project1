...
Problem::Problem(vector<int> initialState, int trenchNum, vector<int> recess) {
    numTrenches = trenchNum;
    recessPos = recess;
    totalSize = numTrenches + recessPos.size();
    state = initialState; 
    int x = state.size();
...
void Problem::printArray() const {
    ...
    for(int i = 0; i < recessPos.size(); ++i) {
        int v = state[numTrenches + i];
        cout << "Recess " << (numTrenches + i + 1) << " at cell " << (recessPos[i] + 1) << endl;
    }
    cout << endl;
}
